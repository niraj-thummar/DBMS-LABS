...
--Or ‘M.G. ROAD’ and Account No is less than 104 from deposit table.
    SELECT ACTNO,CNAME,AMOUNT FROM DEPOSIT WHERE BNAME IN ('AJNI','KAROLABAUG','M.G. ROAD') AND ACTNO < 104;

--15. Display all loan no, customer from borrow table does not belong to ‘VIRAR’ or ‘AJNI’ branch. (use NOT
--IN)
    SELECT LOANNO,CNAME FROM BORROW WHERE NOT BNAME IN('VIRAR','AJNI');
...
--number is not a multiple of 3.
    SELECT * FROM BORROW WHERE AMOUNT > 3000 AND AMOUNT % 3 <> 0;
